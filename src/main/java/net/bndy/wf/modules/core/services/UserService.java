...
public class UserService extends _BaseService<User> {
    ...
    public User register(User user) {
		if (findByUsername(user.getUsername(), user.getId()) != null) {
			return null;
		}
		
		User oldUser = findByUsername(user.getUsername(), null);
		UserProfile oldProfile = userProfileRepository.findByUserId(oldUser.getId());

		oldProfile.setRealName(user.getUserProfile().getRealName());
		oldProfile.setGender(user.getUserProfile().getGender());
		oldProfile.setEmail(user.getUserProfile().getEmail());
		oldProfile.setBirthday(user.getUserProfile().getBirthday());
		
		oldUser.setUsername(user.getUsername());

        user = userRepository.saveAndFlush(oldUser);
        userProfileRepository.saveAndFlush(oldProfile);
    ...
    public User findByUsername(String username, Long id) {
    	if(null != id) {
    		return this.userRepository.findByUsernameAndIdNot(username, id);
    	} else {
